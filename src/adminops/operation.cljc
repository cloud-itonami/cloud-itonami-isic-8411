(ns adminops.operation
  "OperationActor -- one public-administration operation = one
  supervised actor run, expressed as a langgraph-clj StateGraph. The
  advisor (AdminOps-LLM) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the Public Administration Governor
  ...
  One graph run = one public-administration operation (intake -> advise
  ...
  decision to a human operator (case officer/authorized official). The
  approver resumes with `{:approval {:status :approved}}` (or
  :rejected). `:case/decide`/`:case/notify` ALWAYS reach this node
  when the governor is clean -- see `adminops.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [adminops.adminopsllm :as adminopsllm]
...
(defn build
  ...
  `adminops.store/Store`).
  opts:
    :advisor      -- an `adminops.adminopsllm/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (adminopsllm/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; AdminOps-LLM inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (adminopsllm/-advise advisor store request)]
            {:proposal p :audit [(adminopsllm/trace request p)]})))

      ;; Public Administration Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
